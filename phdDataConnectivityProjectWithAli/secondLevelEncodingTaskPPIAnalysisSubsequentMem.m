function secondLevelEncodingTaskPPIAnalysisSubsequentMem(regionName,subID)
% List of open inputs
...
load(['C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\s001\encoding\analysis-subsequentMemory\',...
...
for nFolder = 1:size(P.Contrasts,2)
    ...
    if ~exist([rootDir,'secondLevelPPIAnalysisSubsequentMem\',regionName,'\',P.Contrasts(nFolder).name],'dir')
        mkdir([rootDir,'secondLevelPPIAnalysisSubsequentMem\',regionName,'\',P.Contrasts(nFolder).name]);
    end
    inputs{1,1} = {[rootDir,'secondLevelPPIAnalysisSubsequentMem\',regionName,'\',P.Contrasts(nFolder).name,'\']}; % Factorial design specification: Directory - cfg_files
    ...
    for n = 1:length(subID)
        ...
        inputs{2, 1}(n) = {[rootDir,subjDir(n).name,'\encoding\analysis-subsequentMemory\','PPI_', regionName,'\',...
