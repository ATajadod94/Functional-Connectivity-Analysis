function pmod = makeMultiPmodFile(subID, dataName,isFamOnly,isBinarized)
...
if strcmp(dataName, 'pca')
    load 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\matlabFunctions\priorFaceRatingPCAData.mat'
    data(:,1:2) = [fam1PCA(:,subID),fam2PCA(:,subID)];
end

if strcmp(dataName, 'pca50percentile')
    load 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\matlabFunctions\priorFaceRatingPCAData.mat'
    data(:,1:2) = [fam1PCA(:,subID),fam2PCA(:,subID)];
end
...
elseif strcmp(dataName, 'pca')
elseif strcmp(dataName, 'pca50percentile')
else
...
elseif strcmp(dataName, 'pca')&& isBinarized ~=1
    data = data-repmat(mean(data),size(data,1),1);
elseif isBinarized ==1 && strcmp(dataName, 'pca') ~=1 && strcmp(dataName, 'pca50percentile') ~=1;
    data(find(data<4))=-1;
    data(find(data>3))=1;
    
elseif isBinarized ==1 && strcmp(dataName, 'pca') ==1;
    data1=data;
    for m = 1:2
   ...
   data=data1; 
elseif isBinarized ==1 && strcmp(dataName, 'pca50percentile') ==1;
    data1=data;
    for m = 1:2
...
% save with one flag
% save([behavDataDir,dataName,'multiCondPmod-',num2str(isFamOnly),'.mat'],'names','durations','onsets',...
%     'pmod');

%save with two flags

save([behavDataDir,dataName,'multiCondPmod-',num2str(isFamOnly),num2str(isBinarized),'.mat'],'names','durations','onsets',...
    'pmod');
