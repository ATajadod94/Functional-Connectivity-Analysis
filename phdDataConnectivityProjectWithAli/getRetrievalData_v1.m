...
%****** this program has bugs, and cannot be used!!!!
...
if ~isempty(idRecombCorrectIndex) % as long as correctly identified, the recombined pairs will be treated as correct associative memory. the number are similar to intact hit
    houseCorrectRecom = txtData(idRecombCorrectIndex, idColc);
    faceCorrectRecom = txtData(idRecombCorrectIndex, idCold);
    idCorrectRecomBoth=idRecombCorrectIndex;
    % take out the condition that only both crossed recognized pairs being
    % recognized the pairs were treated as correct associative memory
