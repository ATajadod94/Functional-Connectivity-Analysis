function [yfam, ynfam fh sc fh_sc] = plotEasyDiffPicVsScrambleBarGraphs_anterioPosteriorHPC(isplot)
if nargin<1
    isplot=1;
end
addpath C:\Users\Zhongxu\Documents\MATLAB\myMatLabTools\majorNeuroImagingTools\marsbar-0.43
famDir = 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\secondLevelPmodAnalysis\easyDifficultPic&ScrmonlyGoodMemSubj\famnfam12\';
nfamDir = 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\secondLevelPmodAnalysis\easyDifficultPic&ScrmonlyGoodMemSubj\famnfamscrmble_12\';
...
rois = { 'L_aHC.img','R_aHC.img','L_pHC.img','R_pHC.img', 'L_vmP.img', 'R_vmP.img'};
% rois = { 'L_vmP.img', 'R_vmP.img'};
yfam = getRawY(famDir);
ynfam = getRawY(nfamDir);

[yfam,xRm,idRm,idRmi]=replaceOutliers(yfam,2.5,2);
[ynfam,xRm,idRm,idRmi]=replaceOutliers(ynfam,2.5,2);

[a fh.p fh.ic fh.stats]=ttest(yfam,0);
[a sc.p sc.ic sc.stats]=ttest(ynfam,0);
fh.m=mean(yfam);
sc.m=mean(ynfam);
[a fh_sc.p fh_sc.ic fh_sc.stats]=ttest(yfam,ynfam);
...
if isplot==1
    plotBarFig(my_barvalues',my_errors','left',myFontSize,'beta',figDir,[8,5]);
    plotNotBoxPlot(yfam,figDir,'easydiffEncodingapHPCvmPFC',2);
end

my_barvalues = [mean(ynfam(:,1:2:length(rois)));mean(ynfam(:,2:2:length(rois)))];

my_errors =  [std(ynfam(:,1:2:length(rois)))/sqrt(16);std(ynfam(:,2:2:length(rois)))/sqrt(16)];
if isplot==1
    plotBarFig(my_barvalues',my_errors','right',myFontSize,'beta',figDir,[8,5]);
    plotNotBoxPlot(ynfam,figDir,'easydiffPerceptualapHPCvmPFC',2);
end
