function plotEncodingEncodingFHScrambleEffectSingleBarGraphs(contrastName)
if nargin==0
    contrastName = 'picEffect_Pic-Scramble';
end
famDir = ['C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\secondLevelAnalysis\',contrastName,'\'];
...
%     plotBarFig(my_barvalues',my_errors',[rois{i},'_encodingPicEffectsinglebar'],myFontSize,'beta',figDir,[4,4]);
%     
% end
my_barvalues = [mean(yfam(:,1:2:10));mean(yfam(:,2:2:10))];

my_errors =  [std(yfam(:,1:2:10))/sqrt(20);std(yfam(:,2:2:10))/sqrt(20)];
plotBarFig(my_barvalues',my_errors',['encodingPicEffectsinglebar-',contrastName],myFontSize,'beta',figDir,[8.5,4]);
