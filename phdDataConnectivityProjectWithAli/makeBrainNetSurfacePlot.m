function makeBrainNetSurfacePlot (dataTImage,savefigName,savefigNote, outDir, mimTValue, maxTValue,viewtype, clusterSize)
% This function call rest_CallBrainNetViewer to automatically make surface
% plots. using afni 12 colormap 
[BrainNetViewerPath, fileN, extn] = fileparts(which('BrainNet.m'));
SurfFileName=[BrainNetViewerPath,filesep,'Data',filesep,'SurfTemplate',filesep,'BrainMesh_ICBM152_smoothed_tal.nv'];

if nargin < 5
viewtype='FullView'; % or use 'MediumView','FullView'
clusterSize = 10;
maxTValue = 6;
mimTValue = 1.75; % p = .05, no correction
end
afniColorNumber = 12;
NMim = -mimTValue; 
PMim =  mimTValue; 
NMax = -maxTValue;
PMax =  maxTValue;
 ColorMap = AFNI_ColorMap(afniColorNumber);
% ColorMap = jet(160);
H_BrainNet = rest_CallBrainNetViewer(dataTImage,NMim,PMim,clusterSize,18,SurfFileName,viewtype,ColorMap, NMax,PMax);

 export_fig([outDir,savefigName],'-tiff','-r300');
% plot2svg([savefigName,'-tiff.svg'],H_BrainNet,'pdf');

% exportfig(H_BrainNet, [outDir,savefigName],'Format','png','Color','rgb','Resolution',600);
% exportfig(H_BrainNet, [outDir,savefigName],'Format','pdf','Color','rgb','Resolution',600);
% 

fid=fopen(sprintf('%s',[outDir,savefigName,'notes.txt']),'w');
 if(fid)  
     fprintf(fid,'notes = %s\n',savefigNote);     
     fclose(fid);
 end
