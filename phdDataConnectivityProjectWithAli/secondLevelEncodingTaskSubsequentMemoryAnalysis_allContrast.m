function secondLevelEncodingTaskSubsequentMemoryAnalysis_allContrast(subID)
% List of open inputs
...
contrastName={'remPic-Scramble','remFameEff_fam-nonfam','remFam',...
    ...
    '','nonremFam','nonremNonFam','nonremFam1',...
...
for nFolder = [1:13 15:length(contrastName)]
    if ~exist([rootDir,'secondLevelSubsequentMemoryAnalysis_AllSubj\',contrastName{nFolder}],'dir')
        mkdir([rootDir,'secondLevelSubsequentMemoryAnalysis_AllSubj\',contrastName{nFolder}]);
    end
    
    inputs{1,1} = {[rootDir,'secondLevelSubsequentMemoryAnalysis_AllSubj\',contrastName{nFolder},'\']}; % Factorial design specification: Directory - cfg_files
    for n = 1:length(subID)
        subj = subID(n);
        inputs{2, 1}(n) = {[rootDir1,subjDir(subj).name,'\encoding\analysis-subsequentMemory\con_',sprintf('%04d',nFolder),'.img']};
        % Factorial design specification: Scans - cfg_files
    end
