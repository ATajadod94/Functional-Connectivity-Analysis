function secondLevelvoxelPmodAnaysisEasyDiffPicScrm(dataName,isFamOnly,includedSubj)
% List of open inputs
...
for nFolder = 1:length(contrastName)
    ...
    for n = 1:length(includedSubj)
        inputs{2, 1}(n) = {[rootDir,subjDir(includedSubj(n)).name,...
            '\encoding\analysis-',dataName,'Pmod-',num2str(isFamOnly),'\con_',sprintf('%04d',contrastNum(nFolder)),'.img']};
        % Factorial design specification: Scans - cfg_files
    end
