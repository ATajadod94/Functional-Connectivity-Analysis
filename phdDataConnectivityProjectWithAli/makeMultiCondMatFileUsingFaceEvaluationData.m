function BinaryExcluded = makeMultiCondMatFileUsingFaceEvaluationData(subID,dataName,isFamOnly)
...
BinaryExcluded=0;
for ii=1:2 % find missing condiition participant, exclude them
    if isempty(find(data(:,ii)==1))
%         data(end,ii)=1;
BinaryExcluded=1;
        disp([subID,ii]);
...
for ii=3:4 % find missing nonfam condition, add one trial, not analyzed anyway
    if isempty(find(data(:,ii)==1))
        data(end,ii)=1;
