function [idBlockE idBlockD idExclude] = getMemoryPerformanceForEachTrial_0123_notBalanced(memId)
if nargin<1
    memId=[0 1 2 3];
end
nsubj=20;
...
[aa ia ib]=intersect(a{1},b{1},'rows','stable');
c=stimEncode1.nfmpair(:,1);
d=stimEncode1.nfmShuffled{1}(:,1);
[aa ic id]=intersect(c{1},d{1},'rows','stable');
...
%         idD={};
        for k=1:length(memId)
            ...
            if ~isempty(intersect(idE{k},idD{k}))
                disp('eay diff trial id overlap')
                disp([i j k])
            end
%             if minTrialNum==0
%                 idE{k} = [];
