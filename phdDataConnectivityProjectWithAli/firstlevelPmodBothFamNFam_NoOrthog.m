function firstlevelPmodBothFamNFam_NoOrthog(subID,dataName,isFamOnly,isBinarized)
if nargin <4
    isBinarized =[];
end
% List of open inputs
...
subDir = [rootDir1, 's',sprintf('%03d',subID),'\'];
...
inputs = cell(26, nrun);
for crun = 1:nrun
    FUNCDIR = [rootDir1,'s',sprintf('%03d',subID(crun)),'\encoding'];
    inputs{1, crun} = {[FUNCDIR,'\analysis-',dataName,'Pmod-',num2str(isFamOnly),num2str(isBinarized),'_NoOrthog\']}; % fMRI model specification: Directory - cfg_files
    if ~exist([FUNCDIR,'\analysis-',dataName,'Pmod-',num2str(isFamOnly),num2str(isBinarized),'_NoOrthog\'],'dir')
        mkdir([FUNCDIR,'\analysis-',dataName,'Pmod-',num2str(isFamOnly),num2str(isBinarized),'_NoOrthog\']);
    end
