function [p t baselineCorrGrpLR postFamCorrGrpLR postNonFamCorrGrpLR] = ...
    prepostFamEncodingCorr(subjID,isHemiLR,isControlCov,filterBand)
if nargin <4
    filterBand=[];
end
projDir = 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\';
figOutPath = 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\pre_postFam_EncodingCorr_5ROI\';
if isControlCov==1
    figOutPath = 'C:\Users\Zhongxu\Documents\myStudy\picpairfMRI\pre_postFam_EncodingCorr_ControlCov_5ROI\';
end
...
for i = 1:nRoi
    for j = 1:nRoi
        [a p(i,j) ci stats]=ttest(postFamCorrGrpLR(i,j,:),baselineCorrGrpLR(i,j,:));
...
s=figure;
figTitle = {['PostFamous vs Baseline correlations',hemiSide]};
...
set(gcf,'color','w');
colorbar;
export_fig([figOutPath,'PostFam_Baseline_',hemiSide],'-tiff','-r300');
...
imagesc(tFdr_05);
caxis([-5 5]);
title('PostFamous-Baseline diff t value');
set(gca,'XTick',[1 : nRoi],'YTick',[1 : nRoi]);
set(gca,'XTickLabel',labelLR,'YTickLabel',labelLR);
set(gcf,'color','w');
colorbar;
...
export_fig([figOutPath,'PostFam_Baseline_diff_TvalFdr05_',hemiSide],'-tiff','-r300');
...
export_fig([figOutPath,'PostFam_Baseline_diff_TvalFdr05Trend_',hemiSide],'-tiff','-r300');
...
export_fig([figOutPath,'PostFam_Baseline_diff_TvalNoCrect_',hemiSide],'-tiff','-r300');
close all;
