function conn_batch_resting_Results
...
batch.Results.foldername='R_PPA';                                   % foldername where second-level results will be stored (this folder will be located in the results/secondlevel/ folder within the project folder) 
    
allSubj=1:20;
batch.Results.between_subjects.effect_names={'subjAll'};   % second-level between-subject effect names: cell array of strings (one per effect)
batch.Results.between_subjects.effects= {allSubj};            % second-level effects (subjects by effects matrix) 
batch.Results.between_subjects.contrast=1;                      % between-subjects contrast (vector of 1 by effects-defined-above values)

batch.Results.between_conditions.effect_names={'postFam','postNonFam'};   % list of condition names: cell array of strings (one per condition), these should correspond to the conditions names defined in the experiment setup 
batch.Results.between_conditions.contrast=[1 -1];               % between-condition contrast (vector of 1 by conditions-defined-above values)

batch.Results.between_sources.effect_names={'R_PPA_1_1'};        % list of seed names: cell array of strings (one per source). these should correspond to the ROI file names -or label names for atlas ROIs- imported in the experiment setup. If in doubt display the "CONN_x.Analyses(ianalysis).sources" variable to see a list of available names for this experiment. note: the _1_1 part indicates the first derivative order/dimension extracted (you would only need to change this if multivariate extraction for a given ROI is performed)
