function [minTrialCount,pmod] = makeMultiPmodEasiNessFile(subID, dataName,isFamOnly,isBinarized)
...
if strcmp(dataName, 'easyDifficultPic&Scrm')
...
if strcmp(dataName, 'performance')
    data(find(data==0))=-1;
elseif strcmp(dataName, 'easyDifficultPic&Scrm')
...
picEasyCount=min(sum(data==1));
scrEasyCount=min(sum(dataSc==1));

picDiffiCount=min(sum(data==-1));
scrDiffiCount=min(sum(dataSc==-1));

minTrialCount = min([picEasyCount, scrEasyCount,picDiffiCount,scrDiffiCount]);
...
pmod(2).name{1} = [dataName,'_Fam1-scrm'];
pmod(2).param{1} = dataSc(:,1);
pmod(2).poly{1} = 1;
...
pmod(4).name{1} = [dataName,'_Fam2-scrm'];
pmod(4).param{1} = dataSc(:,2);
pmod(4).poly{1} = 1;
...
if isFamOnly==0
    ...
    pmod(6).name{1} = [dataName,'_NFam1-scrm'];
    pmod(6).param{1} = dataSc(:,3);
    pmod(6).poly{1} = 1;
    ...
    pmod(8).name{1} = [dataName,'_NFam2-scrm'];
    pmod(8).param{1} = dataSc(:,4);
    pmod(8).poly{1} = 1;
end
% 
