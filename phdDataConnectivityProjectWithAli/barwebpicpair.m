function handles = barwebpicpair(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type,fs,locationString)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = bar(barvalues, width,'edgecolor','k', 'linewidth', .8);
	hold on
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', locationString);
		legend boxoff;
	else
		handles.legend = [];
	end
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', .5);
	...
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', fs, 'xtick',1:numgroups, 'linewidth', .8,'xgrid','off','ygrid','off');
