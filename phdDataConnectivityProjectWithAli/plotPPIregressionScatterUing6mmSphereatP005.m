...
load(fullfile(dataPath{3},voiFname{1}));
...
plot2svg([pathf,'PPIRegMem-', fame,'-',HPCName,'-',voiName{1},'-sphere.svg'],s);
...
load(fullfile(dataPath{4},voiFname{4}));
load(fullfile(dataPath{4},'SPM.mat'));
plot(mean(xY.y,2),SPM.xX.X(:,4),'sb','MarkerSize',2);
H1=lsline
[a b c d STATS]= regress(SPM.xX.X(:,4),[ones(20,1),mean(xY.y,2)]);
rsq=['R^2 = ',num2str(roundoff(STATS(1),2))];
if STATS(3)<.001;STATS(3)=.001;end
