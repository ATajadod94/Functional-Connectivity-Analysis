function firstlevelPmodFamousOnly_mulitiPmod_twoFlags(subID,dataName,isFamOnly,isBinarized)
if nargin <4
    isBinarized =[];
end
% List of open inputs
...
for crun = 1:nrun
    ...
    inputs{3, crun} = {[behavDataDir,cat(2,dataName{:}),'multiCondPmod-',num2str(sum(isFamOnly)),num2str(isBinarized),'.mat']}; % fMRI model specification: Multiple conditions - cfg_files
...
matlabbatch{3}.spm.stats.con.consess{2+(i-1)*4}.tcon.convec = [zeros(1,i+4), 1];
...
matlabbatch{3}.spm.stats.con.consess{3+(i-1)*4}.tcon.convec = [zeros(1,i), 1, 0,0,0, 1];
...
matlabbatch{3}.spm.stats.con.consess{4+(i-1)*4}.tcon.convec = [zeros(1,i), 1, 0,0,0, -1];
