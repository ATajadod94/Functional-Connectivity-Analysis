function secondLevelRegressionOnePredictorGeneral_withCov_inputDir(contrastName,...
...
disp(dataRootDir)
disp('please check the dataSubDir, whether it is')
disp(dataSubDir)
...
for crun = 1:nrun
    ...
    for n = 1:length(subjDir)
        inputs{2, crun}(n) = {[dataRootDir,subjDir(n).name,'\',dataSubDir,contrastName,sprintf('%03d',n),'.img']};
        % Factorial design specification: Scans - cfg_files
    end
