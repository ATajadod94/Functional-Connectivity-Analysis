...
for i=1:numel(fields)
    ...
    Position = 1;
     ...
     MySubjects.(fields{i}).Anatomy.Filename = fullfile(MySubjects.(fields{i}).Folder,'Anatomy');
        MySubjects.(fields{i}).Anatomy.Image =  (dir(MySubjects.(fields{i}).Anatomy.Filename));
    ...
    for j=1:numel(RunNames)
        ...
        allfile = dir(MySubjects.(fields{i}).Runs.(RunNames{j}).Folder);
...
save('MySubjects_Windows.mat','MySubjects');