function secondLevelAnalysis_general(subj, designType, secondLevelDir,firstlevelName)
%-----------------------------------------------------------------------
...
contrastName = {'facehouse','scrambled','face-scrambled','fam1','scramb1','fam2','scramb2','nonfam1','scramb3','nonfam2','scramb4'};


for nFolder = 1:length(contrastName)
    contrastDir = {fullfile(secondLevelDir,designType,contrastName{nFolder})}; % Factorial design specification: Directory - cfg_files
   
    if ~exist(contrastDir{1},'dir')
        mkdir(contrastDir{1});
    end
    ...
    for n = subj
        m=m+1;
        firstLevelDir = %fullfile(dataFDir,['subj', sprintf('%02d',n)],firstleveName);
        
        % Factorial design specification: Scans - cfg_files
        imageInput(m,1) = {fullfile(firstLevelDir,['con_',sprintf('%04d',nFolder),'.img'])}; % re-smoothed or not
    ...
    matlabbatch{1}.spm.stats.factorial_design.dir = contrastDir;
    matlabbatch{1}.spm.stats.factorial_design.des.t1.scans = imageInput;
    ...
    matlabbatch{5}.spm.stats.results.conspec.extent = 10;
