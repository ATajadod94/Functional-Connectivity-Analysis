function analysislog = doFirstLevelFaceHouseTaskAnalysis_Native(subjN)
% List of open inputs
...
for subject = subjN
    ...
    outdir = fullfile(thissubjectdir,'firstlevelAnalysis_Native'); %zhongxu added '_Native'
    mkdir(outdir)
    ...
    for i=1:numsession
        ...
        for j = 1:length(allfiles)
            if strcmp(allfiles(j).name(1),'r') && strcmp(allfiles(j).name(end-3:end),'.img') %zhongxu change 's" to 'r'
