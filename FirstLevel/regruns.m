...
for subject = 1:20
    ...
    for i=1:numsession
        ...
        for j = 1:length(allfiles)
            if startsWith(allfiles(j).name,'s') && endsWith(allfiles(j).name,'.img')
                sessfiles{1,end+1} = fullfile(thisdir,allfiles(j).name);
            end
            if endsWith(allfiles(j).name,'.txt')
                reg = fullfile(thisdir,allfiles(j).name);
            end                
        end
        files{i} = sessfiles;
        multi{i} = {reg};
